...
export class UsuarioService {
  ...
  //   console.log("aca llamamos")
  //   this.util.removeAllStorage();
  //   return this.http.post<any>(this.baseUrl + "/login-persona",JSON.stringify(data) /*, this.httpOptions*/);
  // }
  ...
  return this.http.post<any>(
    `${this.baseUrl}/login-persona`,data,
    {
      headers: new HttpHeaders({
        'Content-Type': 'application/json'
      })
  ...
  getLoginContribuyente(tipo:number, data:PersonaRequest){
    let info:any;
    if(tipo == 1){
      info = this.http.post<any>(this.baseUrl + "/login-persona",JSON.stringify(data));
    }else{
      info = this.http.post<any>(this.baseUrl + "/login-contribuyente",JSON.stringify(data));
    }
    return info;
  }
